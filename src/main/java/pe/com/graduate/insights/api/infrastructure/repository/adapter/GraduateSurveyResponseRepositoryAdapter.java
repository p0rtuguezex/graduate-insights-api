            ...
            responseEntity.setResponses(
                request.getResponses().stream()
                    .map(questionResponse -> {
                        var questionResponseEntity = graduateSurveyResponseMapper.toEntity(questionResponse);
                        questionResponseEntity.setGraduateSurveyResponse(responseEntity);
                        
                        // Procesar las opciones seleccionadas si existen
                        if (questionResponse.getSelectedOptionIds() != null && !questionResponse.getSelectedOptionIds().isEmpty()) {
                            List<QuestionOptionEntity> selectedOptions = questionResponse.getSelectedOptionIds().stream()
                                .map(optionId -> questionOptionRepository.findById(optionId)
                                    .orElseThrow(() -> new NotFoundException("Opción no encontrada con ID: " + optionId)))
