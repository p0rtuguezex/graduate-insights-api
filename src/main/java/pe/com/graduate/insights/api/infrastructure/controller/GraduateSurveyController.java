...
public class GraduateSurveyController {
    ...
    @Operation(
            ...
            description = "Obtiene las encuestas disponibles para el graduado autenticado con filtros opcionales"
    )
    @ApiResponses(value = {
            @ApiResponse(
                    responseCode = "200",
                    description = "Encuestas obtenidas exitosamente",
                    content = @Content(schema = @Schema(implementation = GraduateSurveyListResponse.class))
            ),
            @ApiResponse(
                    responseCode = "400",
    ...
    public ResponseEntity<ApiResponseWrapper<List<GraduateSurveyListResponse>>> getSurveys(
            @Parameter(
                    description = "Filtro para el estado de las encuestas. Valores válidos: 'all' (todas), 'completed' (completadas), 'pending' (pendientes)",
                    example = "all"
            )
    ...
    @Operation(
            ...
            description = "Obtiene el detalle completo de una encuesta específica con preguntas y respuestas (si las hay) del graduado autenticado"
    )
    @ApiResponses(value = {
            @ApiResponse(
                    responseCode = "200",
                    description = "Detalle de la encuesta obtenido exitosamente",
                    content = @Content(schema = @Schema(implementation = GraduateSurveyDetailResponse.class))
            ),
            @ApiResponse(
                    responseCode = "401",
                    description = "No autorizado"
            ...
            @ApiResponse(
                    responseCode = "404",
                    description = "Encuesta o graduado no encontrado"
            )
    })
    @GetMapping("/{surveyId}")
    public ResponseEntity<ApiResponseWrapper<GraduateSurveyDetailResponse>> getSurveyDetail(
        ...
        Long graduateId = getAuthenticatedGraduateId();
        GraduateSurveyDetailResponse surveyDetail = graduateSurveyUseCase.getSurveyDetailForGraduate(surveyId, graduateId);
        
        return ResponseEntity.ok(ApiResponseWrapper.success("Detalle de la encuesta obtenido exitosamente", surveyDetail));
    }
    ...
    private Long getAuthenticatedGraduateId() {
        ...
        var graduate = graduateRepository.findByUserIdAndUserEstado(user.getId(), ConstantsUtils.STATUS_ACTIVE)
                .orElseThrow(() -> new NotFoundException("El usuario autenticado no es un graduado o no está activo"));
