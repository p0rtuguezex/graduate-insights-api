...
public interface SurveyRepository extends JpaRepository<SurveyEntity, Long> {
    ...
    // Métodos para fechas
    @Query("SELECT s FROM SurveyEntity s WHERE s.startDate <= :now AND (s.endDate IS NULL OR s.endDate >= :now) AND s.status = :status")
    List<SurveyEntity> findActiveInPeriod(@Param("now") LocalDateTime now, @Param("status") SurveyStatus status);
    
    @Query("SELECT s FROM SurveyEntity s WHERE s.endDate < :now AND s.status = :status")
    List<SurveyEntity> findExpiredSurveys(@Param("now") LocalDateTime now, @Param("status") SurveyStatus status);
