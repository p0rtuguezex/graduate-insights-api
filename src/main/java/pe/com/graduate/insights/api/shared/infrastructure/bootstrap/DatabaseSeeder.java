...
public class DatabaseSeeder implements CommandLineRunner {
    ...
    try {
      Long count =
          jdbcTemplate.queryForObject(
              String.format("SELECT COUNT(*) FROM %s", tableName), Long.class);
            ...
            .map(
                existing -> {
                  if (existing.getContrasena() == null || existing.getContrasena().isBlank()) {
                    ...
                    log.info(
                        "Admin director password was empty, re-encoded for {}", normalizedEmail);
                  }
                  return existing;
                })
            .orElseGet(
  ...
  private void seedMockDataIfNeeded() {
    ...
    createEmployer(
        ...
        mockPassword,
        "20601234567",
        "TechIca Solutions SAC",
        "Av. Industrial 100, Ica",
        "Empresa de desarrollo de software en la región Ica");

    createEmployer(
        "Patricia",
        "Rojas Huanca",
        "patricia.rojas@constructora.pe",
        "F",
        "1980-09-12",
        "41222222",
        "987222002",
        mockPassword,
        "20701234568",
        "Constructora IcaSur SRL",
        "Av. Los Maestros 200, Ica",
        "Empresa de construcción civil en el sur del país");
    ...
    jdbcTemplate.update(
        con -> {
          PreparedStatement ps =
              con.prepareStatement(INSERT_USUARIO, Statement.RETURN_GENERATED_KEYS);
            ...
            type -> {
              if ("EMPLOYMENT".equals(type.getName())) {
                ...
                survey.setDescription(
                    "Evalúa la situación laboral de los egresados al primer año de graduarse");
                ...
                survey.setDescription(
                    "Mide la satisfacción de los egresados con su formación universitaria");
                survey.setSurveyType(type);
                survey.setStatus(SurveyStatus.ACTIVE);
                ...
                survey.setDescription(
                    "Seguimiento del desarrollo profesional de los egresados de la promoción 2023");
                survey.setSurveyType(type);
  ...
  private void seedEventsIfNeeded() {
    ...
    long feriaTipoId =
        (Long)
            eventTypes.stream()
                .filter(t -> "Feria de Empleo".equals(t.get("nombre")))
                .findFirst()
                .map(t -> t.get("id"))
                .orElse(eventTypes.get(0).get("id"));
    long conferenciaTipoId =
        (Long)
            eventTypes.stream()
                .filter(t -> "Conferencia Tecnológica".equals(t.get("nombre")))
                .findFirst()
                .map(t -> t.get("id"))
                .orElse(eventTypes.get(0).get("id"));
  ...
  private void seedEmploymentSurveyQuestions(Long surveyId) {
    ...
    long q3 =
        insertQuestion(
            surveyId,
            "¿Su trabajo está relacionado con su carrera profesional?",
            "SINGLE_CHOICE",
            true);
    ...
    long q4 =
        insertQuestion(
            surveyId,
            "¿Cuánto tiempo le tomó conseguir su primer empleo tras graduarse?",
            "SINGLE_CHOICE",
            true);
    ...
    long q5 =
        insertQuestion(
            surveyId, "¿Cuál es su modalidad de trabajo actual?", "SINGLE_CHOICE", false);
    ...
    long q6 =
        insertQuestion(
            surveyId, "¿Cuál es su rango salarial mensual aproximado?", "SINGLE_CHOICE", false);
    ...
    insertQuestion(
        surveyId,
        "Describa brevemente sus funciones o actividades laborales actuales",
        "TEXT",
        false);
  }
  ...
  private void seedAcademicSurveyQuestions(Long surveyId) {
    ...
    long q2 =
        insertQuestion(
            surveyId,
            "¿La malla curricular fue relevante para el mercado laboral?",
            "SINGLE_CHOICE",
            true);
    ...
    long q4 =
        insertQuestion(
            surveyId,
            "¿La infraestructura y equipamiento de la universidad fue adecuada?",
            "SINGLE_CHOICE",
            true);
    ...
    long q5 =
        insertQuestion(surveyId, "¿Recomendaría esta carrera a otras personas?", "YES_NO", true);

    insertQuestion(
        surveyId, "¿Qué aspecto mejoraría de la carrera o de la universidad?", "TEXT", false);
  }
  ...
  private void seedFollowUpSurveyQuestions(Long surveyId) {
    ...
    long q2 =
        insertQuestion(
            surveyId, "¿Ha iniciado o completado estudios de posgrado?", "SINGLE_CHOICE", true);
    ...
    long q3 =
        insertQuestion(
            surveyId,
            "¿Participa en actividades de extensión o redes de egresados de la universidad?",
            "YES_NO",
            false);
    ...
    long q5 =
        insertQuestion(
            surveyId,
            "¿Qué tipo de apoyo considera que la universidad debería brindar a sus egresados?",
            "MULTIPLE_CHOICE",
            false);
